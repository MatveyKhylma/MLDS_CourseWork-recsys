...
class masked_self_attention(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, timestamp_embedding, n_heads=8, attention_aggregate="concat"):
    ...
    def forward(self, input_tensor, time_matrixes, time_masks):
        """
        Args:
        ...
        time_matrix_K = self.timestamp_embedding(time_matrixes)
        time_matrix_V = self.timestamp_embedding(time_matrixes)
        ...
        time_matrix_K = time_matrix_K.reshape(input_tensor.shape[0], input_tensor.shape[1], input_tensor.shape[1], 
                                              self.n_heads, self.dq)
        time_matrix_K = time_matrix_K.transpose(1, 3)
        time_matrix_V = time_matrix_V.reshape(input_tensor.shape[0], input_tensor.shape[1], input_tensor.shape[1], 
                                              self.n_heads, self.dq)
        time_matrix_V = time_matrix_V.transpose(1, 3)
        
        attention_score_time = time_matrix_K.matmul(torch.unsqueeze(Q,4))
        attention_score_time = torch.squeeze(attention_score_time)
        
        attention_score = attention_score + attention_score_time
        ...
        attention_score_expand = torch.unsqueeze(attention_score, 3)
        multi_head_result_time = torch.squeeze(attention_score_expand.matmul(time_matrix_V).transpose(3,4))        
        multi_head_result = multi_head_result + multi_head_result_time
